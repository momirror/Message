...
- (BOOL)joinMulticastGroup:(NSString *)group withAddress:(NSString *)address error:(NSError **)errPtr
{
	...
	{
		const struct sockaddr_in* nativeAddress = (struct sockaddr_in*)[address4 bytes];
		const struct sockaddr_in* nativeGroup = (struct sockaddr_in*)[group4 bytes];
	...
	{
		const struct sockaddr_in6* nativeGroup = (struct sockaddr_in6*)[group6 bytes];
	...
	if(theCurrentSend != nil)
	{
		...
		if([self canAcceptBytes:theSocket])
		{
			...
			else
			{
				...
				result = sendto(theNativeSocket, buf, bufSize, 0, (struct sockaddr *)dst, dstSize);
			}
